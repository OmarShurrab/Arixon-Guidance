...
export function validateQuestionDraft(
  ...
  if (draft.calculation) {
    ...
    // Check if question text has obvious numerical physics calculations without calc metadata
    const containsNumbers = /\d+/.test(draft.questionText);
    const isPhysicsOrMath = draft.subject === 'physics' || draft.subject === 'math';
    if (containsNumbers && isPhysicsOrMath) {
      warnings.push('سؤال يحتوي على أرقام وحسابات ولكن لم يتضمن كائن تحقق برمجي تفصيلي.');
      calcScore = 10;
    }
  }
