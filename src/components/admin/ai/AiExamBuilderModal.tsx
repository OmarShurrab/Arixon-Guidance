...
export const AiExamBuilderModal: React.FC<AiExamBuilderModalProps> = ({
  ...
  const currentSumPoints = approvedQuestions.reduce((acc, q) => acc + (q.suggestedPoints || 5), 0);
  ...
  const canSave = questionCount > 0 && allHaveAnswers;
  const canPublish = canSave && pointsMatch && calculationsPass && noDuplicates;
  ...
  const handleSaveExam = async (publishImmediately: boolean) => {
    ...
    try {
      const examData: Omit<Exam, 'id'> = {
        ...
        subjectNameAr: defaultSubject === 'physics' ? 'الفيزياء' : defaultSubject === 'math' ? 'الرياضيات' : 'المبحث المعتمد',
        lesson: defaultLesson.trim(),
